...
def analysis(doc_ids, scores, calculator, save_path=""):
    ...
    print("exporting top 100 tf words as 'tf.txt'")
    ...
    print("exporting top 100 df words as 'tf.txt'")
    ...
    with open(file_dir, "w", encoding="utf-8") as f:
        ...
        f.write("\n".join("%0.1f %s" % (deviation(score), doc_id)
                           for doc_id, score in temp[:n]))
    print(f"exporting top {n} documents that has the largest deviation of its top 10 words by score as 'diff.txt'")
    ...
    print("exporting documents ordered by its # of words with score 100 as 'doc.txt'")
    ...
    with open(file_dir, "w", encoding="utf-8") as f:
        f.write("\n".join(heapq.nlargest(10,
                                         calculator.words,
                                         key=lambda x: len(x))))
    print("exporting top 10 words that has largest word length as 'long.txt'")
    ...
    with open(file_dir, "w", encoding="utf-8") as f:
        f.write("\n".join([word 
                           ...
                           if len(word) == 1]))
    print("exporting all words that have length of 1 as 'one.txt'")
